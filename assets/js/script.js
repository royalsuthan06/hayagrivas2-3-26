...
// 7. Background Slideshow Fade on Scroll
window.addEventListener("scroll", () => {
    ...
    if (slideshow) {
        ...
        slideshow.style.opacity = fadeValue;
    }
});

// 8. Fullscreen Slideshow Logic (Optional, can be removed if not needed)
const hSlideshow = document.querySelector(".hero-slideshow");
const fsBtn = document.getElementById("fullscreenBtn");
if (fsBtn && hSlideshow) {
    fsBtn.addEventListener("click", () => {
        if (hSlideshow.requestFullscreen) {
            hSlideshow.requestFullscreen();
        } else if (hSlideshow.webkitRequestFullscreen) {
            hSlideshow.webkitRequestFullscreen();
        }
    });
}
